...
extension ECDSA {
    public class Der {
        ...
        internal static func encodeInteger(_ number: CS.BigInt) -> String {
            var hexadecimal = BinaryAscii.hexFromInt(abs(number))
            if (number < 0) {
                let bitCount = 4 * hexadecimal.count
                let twosComplement = CS.BigInt(pow(Double(2), Double(bitCount))) + number
        ...
        private static func parseInteger(_ hexadecimal: String) -> CS.BigInt {
            let integer = BinaryAscii.intFromHex(hexadecimal)
            let bits = BinaryAscii.bitsFromHex(String(hexadecimal.prefix(1)))
            if (String(bits.prefix(1)) == "0") {
                return CS.BigInt(integer)
            }
            let bitCount = 4 * hexadecimal.count
            return CS.BigInt(integer - CS.BigInt(NSDecimalNumber(decimal: pow(2, bitCount)).intValue))
        }
        ...
        private static func generateLengthBytes(hexadecimal: String) -> String {
            let size = CS.BigInt(floor(Double(hexadecimal.count) / 2))
            let length = BinaryAscii.hexFromInt(size)
            if size < 128 {
                return StringHelper.zfill(length, 2)
            }
            let lengthLength = 128 + CS.BigInt(floor(Double(length.count) / 2))
